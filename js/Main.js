document.addEventListener("DOMContentLoaded", () => {
            ...
            async function mostrarSugerencias(input, query) {
                try {
                    ...
                    const artistas = (data && data.results && data.results.artistmatches && data.results.artistmatches.artist) || [];
    ...
    function cerrarSugerencias(input) {
        const wrapper = input.closest(".barra-busqueda, .mobile-buscador");
        var _sl = wrapper && wrapper.querySelector('.sugerencias-lista'); if (_sl) _sl.remove();;
    }
